...
async function hasRemote(cwd, remote) {
  try {
...
// src/commands/git/push.ts
var GitPush = class _GitPush extends Command {
  static description = "Stage all changes, generate an AI commit message, commit, and push to origin.";
  static examples = [
    "forge git push",
  ...
  async run() {
    const { flags } = await this.parse(_GitPush);
  ...
  async runPush(cwd, branch, remote, hasUpstream) {
    try {
...
function formatGitError(action, err) {
